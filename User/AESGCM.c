...
#define _MaxText_Length_ 128  	 //最大加解密数据长度 
...
/*{
	...
	nRet= Encrypt_ByteData(key,key_len,iv,iv_len,hdr,hdr_len,ptx,ptx_len,ctx,tag);
	if(nRet==1)
	{//拷贝密文
		pOutCiphertext[0]=0;
...
nRet=aes_wrap(kek,2,Plaintext,Cipher,contx);
...
int aes_unwrap_string(char* pKey,char* pCipher,char* pPlain)
{
	...
	pPlain[0]=0;
	nRet=aes_unwrap(kek,2,Cipher,Plaintext,contx);
