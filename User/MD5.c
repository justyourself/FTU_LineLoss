...
{
    unsigned long i = 0,index = 0,partlen = 0;
...
//       memcpy(&context->buffer[index],input,partlen);
       RAM_Write(&context->buffer[index],input,partlen);
...
//    memcpy(&context->buffer[index],&input[i],inputlen-i);
    RAM_Write(&context->buffer[index],&input[i],inputlen-i);
}

void MD5Final(MD5_CTX *context,unsigned char digest[16])
{
    unsigned long index = 0,padlen = 0;
...
unsigned char Md5_data(unsigned char *app_buf, unsigned long len,unsigned char *pOutMd5)
{
	...
	MD5Init(&md5);         		
	MD5Update(&md5,app_buf,len);
	MD5Final(&md5,pOutMd5);  
	return 0;
}
